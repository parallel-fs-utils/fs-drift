...
def gen_random_fn(is_create=False):
	...
	if opts.rand_distr_type == file_access_dist.UNIFORM:
		index = random.randint(0, max_files_per_dir) # lower limit 0 means at least 1 file/dir
	...
	if verbosity & 0x20: print 'next file index %u out of %u'%(index, max_files_per_dir)
	dirpath = gen_random_dirname(index)
	fn = os.path.join(dirpath, 'f%09d'%index)
	if verbosity & 0x20: print 'next pathname %s'%fn
	return fn
...
def maybe_fsync(fd):
	...
	if percent > opts.fsync_probability_pct + opts.fdatasync_probability_pct: return
...
def create():
	...
	if verbosity & 0x1000: print 'create %s sz %s'%(fn, target_sz)
	...
	try:
		...
		while total_sz < target_sz:
			...
			assert count > 0
			if verbosity & 0x1000: print 'create sz %u written %u'%(recsz, count)
			total_sz += count
			write_requests += 1
			write_bytes += count
                time_after = time.time()			
...
def append():
	...
	fn = gen_random_fn()
	target_sz = random_file_size()
	if verbosity & 0x8000: print 'append %s sz %s'%(fn, target_sz)
	fd = FD_UNDEFINED
	try:
		...
		while total_appended < target_sz:
			...
			assert recsz > 0
			if verbosity & 0x8000: print 'append rsz %u'%(recsz)
			count = os.write(fd, buf[0:recsz])
			assert count > 0
...
def random_write():
	...
	try:
		...
		while total_write_reqs < target_write_reqs:
			...
			if verbosity & 0x20000: print 'randwrite off %u sz %u'%(off,wrsz)
