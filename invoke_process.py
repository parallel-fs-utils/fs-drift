...
import fsd_log
import common
from common import FsDriftException
...
class subprocess(multiprocessing.Process):
    ...
    def run(self):
        ...
        try:
            ...
            ctrs = wkr.ctx.ctrs
            wkr.ctx = None
            self.sender.send(wkr)
            self.sender.send(ctrs)

# below are unit tests for FsDriftWorkload
...
if __name__ == '__main__':
    ...
    class Test(unittest2.TestCase):
        ...
        def test_multiproc_stonewall(self):
            self.log.info('starting stonewall test')
            self.resetDir()
            ...
            for t in threadList:
                ...
                rtnd_ctrs = t.receiver.recv()
                t.join()
                rtnd_invok.ctrs = rtnd_ctrs
                self.log.info('thread ' + rtnd_invok.tid + ' finished')
                if rtnd_invok.status != ok:
                    raise FsDriftException('subprocess failure for thread %s' % rtnd_invok.tid)
                print('thread %s counters:\n%s' % (t.invoke.tid, str(rtnd_ctrs)))
